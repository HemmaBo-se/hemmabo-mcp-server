...
4. **No silent payment failures.** No `catch { }` with empty/comment-only body in any file under `api/` or `lib/`. Every caught error is logged with structured context and either re-thrown, surfaced as a non-2xx response, or persisted to the booking row as `refund_status = failed` with `refund_error`.
