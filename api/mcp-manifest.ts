...
const MANIFEST_SUMMARIES: Record<string, string> = {
  ...
  "hemmabo_booking_negotiate":
    "Lock a price quote for 15 minutes. Returns a quoteId to use in checkout and guarantees the price will not change.",
  "hemmabo_booking_checkout":
    "Create a booking and Stripe payment. Returns a checkout URL or client_secret for AI agent payment flows.",
  "hemmabo_booking_cancel":
    "Cancel a booking and trigger a Stripe refund according to the host cancellation policy.",
  ...
  "verify_vacation_rental_node":
    "Verify a host-domain Vacation Rental Protocol node by reading /.well-known/vacation-rental.json and its Ed25519 JWKS.",
  "get_verified_stay_offer":
    "Fetch and verify a host-domain signed VRP stay offer with live availability, exact total price, valid_until, and direct booking URL.",
};
...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  res.json({
    ...
    version: pkg.version,
    description:
      "Direct booking infrastructure for vacation rentals. Each host is a sovereign booking node: own domain, 0% commission, payment direct to host via Stripe. Includes Vacation Rental Protocol tools for host-domain signed, AI-agent-readable stay offers.",
    ...
    authentication: {
      ...
      flows: {
        clientCredentials: {
          ...
          scopes: { mcp: "Full access to all MCP tools" },
        },
      },
      registration: {
    ...
    safety_disclosures: {
      ...
      data_collected: [
        "Guest name, email and phone only at checkout",
        "Search parameters such as region, dates, guest count, and host-domain VRP verification inputs",
      ],
      data_sharing:
        "HemmaBo never stores guest payment details. Each booking writes to the host's own Supabase project and Stripe account.",
      external_redirects:
