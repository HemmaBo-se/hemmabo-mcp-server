...
Canonical wire names are `snake_case` and prefixed `hemmabo_` so every MCP client (including claude.ai web, whose tool-name regex is `^[a-zA-Z0-9_-]{1,64}$`) accepts them. The legacy dotted names (`search.properties`, `booking.quote`, …) remain accepted as inbound aliases.
